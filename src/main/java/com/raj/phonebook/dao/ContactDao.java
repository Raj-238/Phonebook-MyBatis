...
public interface ContactDao {
	...
	@Insert("insert into contacts( firstname, lastname, email, phone , city,  address, state, avatar, pincode, country, user_id, created_at) "
			+ " values (#{firstname} , #{lastname}, #{email}, #{phone} , #{city},  #{address}, #{state}, #{avatar}, #{pincode}, #{country}, #{userId}, #{createdAt} )")
	...
	@Update("update contacts set firstname = #{firstname} ,lastname = #{lastname}, email = #{email} , phone = #{phone} , "
			+ "avatar =#{avatar}, address =#{address} , city = #{city} , pincode =#{pincode}, country =#{country}, "
	...
	@Results({ @Result(column = "user_id", property = "userId"),
			@Result(column = "created_at", property = "createdAt") })
	List<Contact> getAllContacts(Integer user);
